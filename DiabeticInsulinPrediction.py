...
def gradientBoosting():
    ...
    text.insert(END,"Extreme Graident Boosting Diabetes Prediction Accuracy : "+str(gbc_acc)+"\n\n")
...
def linearRegression():
    ...
    text.insert(END,"Linear Regression (LR) Insulin Dosage Prediction Accuracy : "+str(lr_acc)+"\n\n")
    print(insulin_y_test.shape)
    age1 = []
    for i in range(len(insulin_y_test)):
        age1.append(i)
    predict = predict.tolist()
    predict.sort()
    plt.scatter(age1, insulin_y_test,  color='black')
    plt.plot(age1, predict, color='blue', linewidth=3)
...
gbButton = Button(main, text="Run Extreme Gradient Boosting Model", command=gradientBoosting)
gbButton.place(x=700,y=250)
gbButton.config(font=font1) 

lrButton = Button(main, text="Run Linear Regression (LR) Algorithm", command=linearRegression)
